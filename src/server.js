...
app.post('/register', async (req, res) => {
    try {
        ...
        if (error.response) {
            console.log(`Error from backend service during registration with status ${error.response.status}`);
            res.status(error.response.status).json(error.response.data);
        } else if (error.request) {
            console.log('No response received from backend during registration');
            res.status(503).json({ message: "Backend service unavailable" });
        } else {
            console.log('Error in making request to backend:', error.message);
...
app.post('/login', async (req, res) => {
    try {
        ...
        if (error.response) {
            console.log(`Error from backend service during login with status ${error.response.status}`);
            res.status(error.response.status).json(error.response.data);
        } else if (error.request) {
            console.log('No response received from backend during login');
            res.status(503).json({ message: "Backend service unavailable" });
        } else {
            console.log('Error in making request to backend:', error.message);
